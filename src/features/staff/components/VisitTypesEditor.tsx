...
const EMPTY: FormState = { name: "", price: 0, isActive: true };
...
export function VisitTypesEditor({
  ...
  const openEdit = (vt: DoctorVisitTypeDto) =>
    setForm({ id: vt.id, name: vt.name, price: vt.price, isActive: vt.isActive });
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col gap-2">
        {[1, 2].map((i) => (
          <div key={i} className="bg-default-100 h-12 animate-pulse rounded-lg" />
        ))}
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-2">
      {visitTypes.map((vt) => (
        ...
        >
          <div className="min-w-0 flex-1">
            <p className="text-sm font-medium leading-tight">{vt.name}</p>
          </div>
          <span className="text-accent shrink-0 text-sm font-semibold tabular-nums" dir="ltr">
            {vt.price.toLocaleString()}
          </span>
          {!readOnly && (
            <div className="flex shrink-0 gap-0.5">
              <Button size="sm" variant="ghost" isIconOnly aria-label={t("common.edit")} onPress={() => openEdit(vt)}>
                <Pencil className="h-3.5 w-3.5" />
              </Button>
              <Button
                size="sm" variant="ghost" isIconOnly aria-label={t("common.delete")}
                isDisabled={remove.isPending} onPress={() => remove.mutate(vt.id)}
      ...
      {form && (
        <div className="border-accent-soft-hover bg-accent/5 mt-1 flex flex-col gap-3 rounded-lg border p-3">
          <TextField value={form.name} onChange={(v) => setForm((f) => f && { ...f, name: v })}>
            <Label className="text-xs">{t("staff.visitTypeName")}</Label>
            <Input placeholder="Consultation" variant="secondary" />
          </TextField>
          ...
          <div className="flex items-end gap-3">
            <NumberField
              ...
              minValue={0} className="flex-1" variant="secondary"
            >
            ...
            <Switch
              ...
              size="sm" className="mb-0.5" aria-label={t("common.status.active")}
            >
      ...
      {!readOnly && !form && (
        <Button variant="ghost" size="sm" className="mt-1 self-start" onPress={openAdd}>
